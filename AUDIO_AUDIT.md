...
seed grains ─► houseBus (directly, bypassing monitorBus)
...
`state.js:buildEnvelopeCurves()` creates `new Float32Array` per call. This is called from `playGrain()` when the cached curves don't match the current parameters. The caching in `_cachedAtk`/`_cachedRel` on the grain params object mitigates this for the cursor path (same params across grains). However, seed grains with `seedWeight < 0.999` create a new params object via `Object.assign(Object.create(cgp), { ... _cachedAtk: null })` (grain.js line 1045), which forces a fresh curve build for every attenuated seed grain.
...
- Seed grains → houseBus directly (audience speakers)
...
2. **Seed grain param cloning** — grain.js line 1044: `Object.assign(Object.create(cgp), {...})` creates a new object per attenuated seed grain. With 4 seeds × 12 grains/tick = up to 48 object allocations per 10ms tick.
...
5. **buildEnvelopeCurves()** — Creates `new Float32Array(128)` pairs. Mitigated by caching on cursor path, but seed grains with `_cachedAtk: null` bypass the cache.
...
main.js (entry point)
  ...
  ├── ui-presets.js     (892 lines)  — Preset management, seed plant/uproot
...
### #3 — Fix Seed Grain Param Object Allocation ✅ FIXED

**File:** `grain.js` (~line 1046–1065)
**Risk:** Creates a new object per attenuated seed grain, bypasses envelope cache
**Fix:** Each seed slot now has a persistent `seed._effectiveParams` object. When `seedWeight < 0.999`, properties are copied in-place via `Object.keys()` loops (no `Object.assign` / `Object.create`). Volume is set as `cgp.volume * seedWeight`. Envelope cache (`_cachedAtk`) is only invalidated when `_lastSeedWeight` actually changes — so curves are rebuilt once per weight change, not per grain.
...
**Risk:** `_buildCandidatePool` and `_buildCandidatePoolRadius` created `new Map()`, `new Set()`, and `[...entries()].sort().slice().map()` intermediate arrays every tick per seed. At 4 seeds × 100 ticks/sec = ~400 Map+Set+sort allocations/sec.
...
**Files:** `grain.js` (angleBetweenSphere, scheduleGrains cursor/seed loops), `renderer.js` (particle creation)
...
**Fix:** Particles now get `_cx, _cy, _cz` (unit-sphere Cartesian) stamped at creation via `stampCartesian(p)`. The cursor/seed angle loops pre-compute the reference point's Cartesian once per tick, then use `_angleFromCached(p, rx, ry, rz)` which is just `acos(dot)` — 1 trig call per particle instead of 7. ~6× reduction in trig operations on cache miss.
