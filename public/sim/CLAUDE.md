...
> **Status: CURRENT — this file is authoritative.** Last verified against the code 2026-09-26 (5.11 alpha). Read this first on every new session, then ONLY the docs the table below marks as relevant to the task *and* CURRENT. If this file disagrees with a doc, this file wins; if it disagrees with the code, **the code wins** — and fix the doc.
...
- **The two engines are TAPE and GRAIN** (Ek, 2026-09-07). Tape plays a take WHOLE, grain in
  fragments; ONE TOOL EACH since 2026-09-22, named for its instrument (`slice` is a tape switch,
  not a tile). **A loop is what a tape take becomes once you pin it**, so `loop` names the PIN KIND
  (`slot.type`, the pins rail, `commitMode`) and only that. The material is `tape`, never `hit` —
  **hit is out of the vocabulary**. `docs/RULINGS.md` "The engines are TAPE and GRAIN".
...
manual/             — the player's cheat sheet (`manual/index.html`), linked from Settings › help and Electron's Help menu; ships beside `js/`
```
...
- **`_held` is what plays, null between presses** (`js/tiles.js`). The engine flags (`traceMode`, `commitMode`, `scanMuted`, `lensReads`, `composerMode`) are the truth underneath; the screen drives them and follows them. `docs/archive/BRUSH-MODEL.md`.
...
- **THE PALETTE IS A FIXED TOOLBAR** (`docs/PALETTE-GUI.md`, read its banner first). Six tiles: the hand's two sides (**tape** · **grain**, wearing their voice, on the spacebar), then **lens · erase · pin · unpin** on `c` `e` `↓` `↑`. Ids and order are the build's, `mubone_palette` stores ONE VERB PER POSITION, **no drag anywhere**, and a tool's id IS its engine's (`engineOf(id) === id`). One action and OSC address per POSITION (`palette_N`). **The mouse SELECTS, the binding PLAYS**: click opens that tile's page, right-click cycles its verb (hand tiles too), the pin pair ignore it. The verb is the shape — `VERB_RADIUS`.
...
- **A stroke freezes the brush that painted it** (`brush-voicing.js`): `resolveGrainParams()` is the ONE builder of a grain block, voicings interned and keyed on the TILE. **The lens owns how the cursor reads** (`k`, order, fill, radius, nearest, recency); the brush owns the sound.
...
Current version: **5.11 alpha** (`5.11.0-alpha` in `package.json`; the chrome shows the minor) **Do not bump the version, touch `CHANGELOG.md`, or push as part of a normal change** (see How we work together). A release is a separate, explicit action Ek initiates ("release" / "bump" / "push", ideally via a release skill). Only then do these five updates apply:
...
| `docs/RULINGS.md` | CURRENT | **The reasoning behind every architecture ruling** — palette, main button, overdub, wash, latency, cloud claims, pin groups, brush/lens split, wet paint, mark timing,. Read the entry for the area you touch; a new ruling goes here as one paragraph |
...
| `docs/TAPE-STUDY-2026-09.md` | PROPOSAL — NOT IMPLEMENTED | **Read before touching the tape sheet or a pin's direction.** 17 loopers: frozen vs live, why speed is varispeed, the proposed sheet |
| `docs/VOICES-AND-HEADS.md` | DESIGN INTENT | The argument behind shape · voice; its words are superseded. **RULINGS is the shipped truth** |
| `docs/FADING-STROKES.md` | PROPOSAL — NOT IMPLEMENTED | The 2026-09-05 study for marks that leave on their own (`life` per mark). Nothing built |
| `docs/CAPS-AND-THROTTLES-2026-09.md` | CURRENT (audit) + PROPOSAL | **Read before touching any grain cap, throttle or budget.** What each protects, whether it still binds, what the gauges cost (measured), the ranked plan P1–P6. § 3 answers "how do we know the machine is at its limit" |
...
| `docs/EXPERIMENTAL-BRUSHES.md` | CURRENT | The #218 inventory, now a record of culls: all four heads are gone — `spray` and `sort by` became grain params and were sunset the same day (2026-09-22), `match` and `staff` deleted. The unit bug, worked. Read before touching `js/paint-ticker.js` |
