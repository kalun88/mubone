...
- **The hand is ONE tool** (`inHand`), played by **space** and a left-click on the sphere in one verb (`handVerb`) that COMES WITH THE TOOL — a strip tile's own, or by engine from the rail (tape toggle, grain and erase momentary), the hand tile's right-click flips it; **the strip is quick access** — a tile fires from its own key in its own verb, never touching the hand. `Tab` shows the tool rail, never a drawer; the row's panel button (`[data-more]`) is the drawer's door. **Every learned key and note is a button** — one recogniser.
