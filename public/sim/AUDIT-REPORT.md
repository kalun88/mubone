...
### 5. Per-Grain Hann Curve Allocation for Cloud Grains (grain.js:112–118)
**Severity: MEDIUM — Allocates 2x Float32Array(128) per cloud grain**
...
Cloud grains (those with `customParams`) allocate two fresh Float32Arrays every call and fill them with scaled Hann values. At dense cloud periods this is thousands of short-lived allocations per second.

**Fix:** Cache the scaled curves per cloud slot. Each cloud has fixed `grainParams.volume`, so compute the curves once when a cloud is dropped (or when its volume changes) and store them on the cloud object.
...
Every 10ms scheduler tick, ALL particles are mapped with an `angleBetweenSphere` call (which does `acos`, `cos`, `sin` — 6 trig calls per particle). With 500 particles, that's 3,000 trig calls per tick × 100 ticks/sec = 300,000 trig ops/sec. The same computation is repeated for each cloud (grain.js:615–617).
...
### 7. Duplicate ensureAudioContext() Calls in Cloud Path (grain.js:630)
...
`ensureAudioContext()` was already called at line 474 (start of `scheduleGrains`). The cloud loop calls it again for every cloud. It's a cheap check (`if (!S.audioCtx)`) but it's unnecessary noise.
...
```js
const cloudCount = S.activeClouds ? ...     // S.activeClouds doesn't exist; it's S.cloudSlots
const sampleCount = S.loadedSamples ? ...   // S.loadedSamples doesn't exist; it's S.samples
```

If `initDiag()` were ever called, the report would show 0 clouds and 0 samples. These reference old property names that were renamed during refactoring.
...
| 5 | MED | grain.js | Cloud curves allocated per-grain | GC pressure |
| 6 | MED | grain.js | Candidate trig computed every 10ms tick | CPU hog |
| 7 | LOW | grain.js | Duplicate ensureAudioContext in cloud loop | Redundancy |
...
2. **Cache reverse buffers and cloud grain curves** (#4, #5) — Eliminates thousands of per-grain allocations. Pre-compute reversed buffers once per sample. Cache scaled Hann curves per cloud.
