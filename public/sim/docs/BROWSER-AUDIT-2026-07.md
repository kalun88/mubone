...
For reference, the keys a clean boot immediately re-writes (so seeing them after a reset is correct, not leakage): `mubone_learn`, `mubone-hud-scale`, `mubone_sensor_cal_v`, `mubone_preset_view`, `mubone_projector_layout_v2`, `mubone_uiScale`.
