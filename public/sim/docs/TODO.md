...
- [ ] **#88 Projection-to-space calibration** — method for aligning the projected sphere visualization to the physical speaker layout in the room. Needs a calibration step so the visual sphere orientation matches the VBAP spatial field from the audience/performer perspective. **Partial (Mar 28):** FOV is now configurable via viz settings slider (20°–120°, default 80°). Tooltip notes projector throw matching. For Nebula Capsule 3 Laser (throw ratio 1.2:1), set to ~45°. Remaining: visual-to-VBAP alignment procedure, aspect ratio matching.
...
- [ ] **#90 Electron fullscreen shows viz only (like browser)** — Electron fullscreen was fullscreening the entire window with GUI panels visible. Fixed: toggles `body.electron-fullscreen` class to hide top bar, right panel, bottom panel, and expand canvas to fill the window, matching the browser `canvasWrapper:fullscreen` behavior.
