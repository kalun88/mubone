...
- [ ] **#114 Sub-10ms grain period: reduce main thread load ("performance mode" v2)** — Investigate lowering `SCHED_SAFE_PERIOD_S` below 10 ms by reducing/disabling the most expensive main thread work during performance. Candidates to hide/mute/throttle when toggled on:
