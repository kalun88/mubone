...
| `/seed/plant` | *(bang)* | Plant a seed at the current cursor position |
| `/seed/uproot` | *(bang)* | Uproot (remove) the nearest seed |
| `/undo` | *(bang)* | Undo the last particle paint action |

Bang-style messages (`/seed/plant`, `/seed/uproot`, `/undo`) trigger on any incoming value — send `1` or bang from Max.
