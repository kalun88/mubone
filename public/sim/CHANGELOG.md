...
Headline: **the palette is the instrument.** Five tiles — cap · lens ‖ loop · grain · erase — and one main button. A slot holds one tool of its kind; tap it to arm, tap again to cycle. Space, a click, the pedal, `/trace` and the instrument's own buttons all press the same button, toggle by default. Every tool is a tile that owns its whole engine block, and a stroke freezes the brush that painted it unless the brush is *wet*.

### Added

#### The tile screen and the palette (#216–#291, #328)
- **One screen.** Chrome, a full-bleed sphere, the pinned rail, the palette, the footer. The rig view's panel columns are gone; the 44 elements the engine pages write through live on in the hidden cabinet.
- **Five tiles, three kind-locked slots, the rail as library.** A rail click loads its kind's slot and opens nothing; Tab / ⇧Tab / the ⋯ are the drawer's only doors and an open drawer follows the hand. Cycle membership is chosen in the rail; a kind's palette is never empty. Digits `1`–`5` tap the palette by position; *tool keys fire* makes `3 4 5` play.
- **Engine sheets** (`renderProps`): one line per parameter, every number typeable, double-click resets to the tick. **Two left rails** overlay the stage and never resize the sphere. **One settings door** with the real modals moved in and back.
- **Shape encodes affordance, one hue per engine, flat surfaces**, written down in `docs/INSTRUMENT-GUI.md`, `docs/SETTINGS-GUI.md` and `docs/DESIGN-SYSTEM.md`, and measured by `align-audit` and the screen probe rather than asserted.

#### Brushes, lenses, pins
- **Brushes:** line, spray, splatter, comb, slice (floor-adaptive auto-segmentation), the looper, the **overdub** (a take inside a pinned loop's cycle, played as a phase-locked layer of that pin, heard pass by pass, #330) and the **wash** (a grain brush whose stroke is pinned at release as a moving cloud — the grain sheet's `on end` row, #334). Three erasers: scrape · bottom · all. Two lenses: wide, spot.
- **Wet paint (#324):** a grain brush switched wet owns one live voicing; every stroke it painted follows its knobs until it dries. Replaces audition and the grain filter.
- **A tile is the preset (#325):** every grain tile owns and persists its whole block; the patch bank is sunset.
- **Pins group by kind** — clouds and loops, mute or solo either; the group is derived and stored nowhere. **A pinned cloud owns its material**: the cursor never granulates inside it. **The reach line is one per candidate.**
- **Flow:** the deposit clock; every stroke deposits marks whatever its brush. **A mark is a moment:** sized by the audio after it, and its grain starts before it by the voice's peak offset (#219 → 2026-09-02).
- **The loop follows the button (#332):** a hit take stamps press and release on the audio clock; its edges are its region; `S.latency` is estimated from the streams or measured by loopback. **The two IPC hops are bounded (#333):** one stall cushion sets the output queue and the input ring.

#### Sensors and instruments
- **The sensor drives the cursor; the camera is derived**, upright-only and pitch-clamped (#300–#306). Roll lock finally works: two writers of `camQ` became one.
- **Mount calibration is two measurements — aim, then bow forward, counted in — and zero heading is a pure twist about vertical** (`sensor-audit`, 36 checks, 200 random mountings).
- **Settings → Sensors rebuilt (round ten, #310–#320):** one card per sensor, one word per transport, `Set mounting` / `Zero heading`, the pills read `● SUBJECT · slot`.
- **First-party instrument support (sygaldry):** over the cable and over Wi-Fi, LED feedback, its three buttons as the palette's first three slots. `docs/BNO085-CONTROL.md` studies the part; `docs/RIG-RUNBOOK.md` is the venue procedure; a diagnostics page runs the measurements.

#### The harness, and how the work is done
- **The dev bridge:** `npm run electron:dev` makes the running app drivable from a script; `scripts/lib/rig.js` launches private instances — in the background, on a fresh profile deleted on close.
- **`rig-audit.js`** runs seven suites against the real app in one boot (action ranges, cc mirrors, trigger, engine pages, mark align, palette, pins). `screen-probe` + `probe-selftest` diff the screen. `docs-audit` holds the docs to their conventions and the two session files to their size. `browser-audit` runs on macOS again (70 checks). `deadweight-audit` inventories what nothing reaches.
- **`audit-for.js`** maps a diff to the one suite to run; **`/finish`** closes a change, **`/release`** cuts one; `worktree-setup.sh` makes a parallel session possible.
- **Docs reorganised:** CLAUDE.md is rules and pointers (32 KB); the reasoning lives in `docs/RULINGS.md`, the audit reasoning in `docs/AUDITS.md`; `docs/TODO.md` is the open list only, done items in `docs/archive/TODO-DONE-<month>.md`; shipped plans are archived the day they ship.

### Changed
- The main button is **one thing** — toggle or momentary, decided above the brush (#329). No tap-versus-hold hybrid.
- `k`, fill and order moved from the brush to the **lens**; aperture retired (#233).
- The sensors, camera and cursor vocabulary: *pin* is the word; *zoom* became *field*; no "wand".
- **Max is a prototyping tool**, not part of the app: two ports, any sender. `proxy.js` is the one relay this repo maintains.
- Releases are explicit; **commits are per change**; plan-before-executing applies to renames, deletions and cross-module refactors.

### Fixed
- **The release glitch (2026-09-02):** a grain keeps its mark and shortens; frontier grains are landed, not cut; a take is sealed after the recorder's last bundle (`whenSealed()`).
- The end of a slice clicked (#331). The chop switch had no dispatch case. The Electron speaker sweep ignored mute. `latency.js` was missing from the service-worker shell. `mubone_pinned_rail` was unregistered. The probe self-test had gone red on live telemetry text.
- The sensors page: `getCalibratedEuler` throw, the Traffic row measuring an unused transport, the rate pill counting every stream, a permission problem reported as a hidden network.

### Removed
- `sandbox/` (git is the graveyard; `docs/archive/SANDBOX.md` is the ledger), the gesture window and `sensor3Cal`, recenter, `debug-waveform.js`, `ui-trace.js`, `mapping_toggle_1–4`, four storage keys, 92 CSS classes, two backup stylesheets, `shortcuts.html`.
- `trace+loop`, the A key and `/trace/mode`, aperture, the pin lenses, the named pin groups, the patch bank, audition, the grain filter, the RO button and the ⌥ cursor-lock mechanism, the projector partition, the perform quick view.
- Seven shipped plan docs and four HISTORICAL docs moved to `docs/archive/`; 39 stale TODO items closed with their reasons.

---

## 1.13 alpha — 2026-08-20

**Covers 2026-08-02 → 2026-08-20 (TODO #177–#200).** One long arc — the trigger tool, taken from a workshop idea to a fully bindable instrument across ~24 iterations driven by playing it back — plus a renderer/worklet/audio performance pass (Aug 3–5) and the OSC inbound audit that the multi-station Max plan depends on.

Headline: **mubone has a second instrument.** Hold **⇧space** instead of space and the buffer you record is a *trigger* — it plays back as a whole sample, verbatim, whenever the cursor touches any part of the stroke, rather than being granulated. It was built for the percussion-ensemble workshop (x-imu3 on a turntable, percussion gestures painted at bearings — triangle at 12 o'clock, woodblock at 3 — firing as the direction vector reaches them), and granulation is simply the wrong primitive for it: no grain-param tweak plays a recording straight through. The tool reuses the existing `loop` commit's playback engine almost wholesale; the genuinely new part is a **proximity gate**. Alongside it, the grain envelope finally honours `fadeRatio`, the paint-deposit clock came off the render frame, and the OSC surface was audited end-to-end ahead of driving mubone from Max.

### Added

#### Trigger tool — samples fired by cursor proximity (#177–#200)
- **A trigger is a *view* onto a painted stroke, not a copy of it.** The particles are the live objects in `S.particles` found by `strokeId`; the buffer is the stroke's own source buffer; the region is `[earliest, latest]` of the surviving particles. `rebuildTrigger()` re-derives everything whenever `S._particleVersion` moves and drops the trigger when its material is gone — so **erase, erase-all, undo, sweep and session import all work with no special cases**: they operate on the particles and the trigger follows. Every painted sound stays granulatable; the type decides which way the cursor plays it, and the type belongs to the recording (space = granular, ⇧space = trigger) — **never both**. No tool mode, no arm/disarm: those were built, then deleted once the view model made them unnecessary.
- **The type belongs to the buffer, decided before you record** — the player knows before pressing record whether a sound will be struck or granulated (Ek's model, arrived at over three rebuilds). Recording a trigger deliberately **does not** mute scan (you may be granulating a section and want to drop a trigger point into it); recording a loop still does. A fresh trigger **plays once on release** as an audition of what you just captured — the cursor is already on the stroke — and then obeys the normal enter/exit rules.
- **The gate is pure geometry and cheap.** `acos` is monotonic, so `angle < gate` becomes `dot > cos(gate)` — the per-particle scan is multiply-add only, with a bounding cap computed once at arm time rejecting everything the cursor isn't near. **32 triggers × 200 particles = 0.0028 ms/tick** against the 20 ms scheduler interval. Hysteresis (exit radius 1.15× enter) and a rearm window (120 ms) ship in v1, not deferred: a turntable parked on a bearing sits exactly on the radius boundary and would otherwise machine-gun the sample. A one-shot deliberately ignores the exit edge, so a fast sweep past the triangle still gets the whole triangle.
- **A trigger's reach is the cursor's search radius (#185)** — not a second radius. Erase and loop-drop already follow `S.searchRadiusDeg`; it is the same cursor and the same gesture, so it has one reach. Riding the radius slider now widens every trigger zone at once.
- **Playback params are live and global, read at fire time (#184).** `S.triggerParams` — dwell, start, release, retrig, speed, volume, hysteresis, rearm, chop — apply to every trigger at once and are switchable mid-phrase; the recording supplies only material and position. `_applyLiveParams()` reaches a trigger that is already sounding (flip dwell to loop while a one-shot rings and it loops), comparing against `_appliedVol`/`_appliedSpeed` rather than the node's mid-ramp `gain.value`.
- **`dwell: once | loop | grain` (#191).** *Sweep past and it fires; stop on it and it does whatever dwell says.* `grain` opens the buffer to the granular cursor (needs SCAN on to be heard) — it needed almost no machinery, because a trigger's zone already is the cursor radius. **`start: top | touch | ends` (#189, #190):** `top` ignores approach direction, `touch` begins where the cursor lands, and `ends` always plays in full and lets the approach pick direction — reach the front half and it runs forwards, the back half and it runs **backwards from the very end**, so on a turntable playback direction follows spin direction. **`retrig: cut | layer` (#193):** `layer` stacks up to 8 voices with no voice engine (a one-shot already carries its own scheduled end), and a stacked voice freezes its firing bearing, so hitting one buffer at different bearings spreads the stack across the speakers.
- **Erase doubles as a sample editor (#186–#187).** Erasing the tail shortens the sample, the head trims it, the middle **splits it into two independent triggers** (each a view onto its own new stroke), and erasing all of it removes the trigger. Keeping the source buffer rather than an extracted copy is what makes this instant. **`chop` (#194–#195)** is the opt-in counterpart: split a take into a sequence of triggers at its silences, so a traced gesture reproduces a phrase note-by-note. Both the erase-split and chop route through `_clusterByRemoval`, which is now **threshold-free** — it splits only where two survivors were not adjacent before, i.e. where something was genuinely removed — so an ordinary 20-second take with rests and breaths in it arms as **one** trigger (the bug in #192, whose fix is the whole point of the distinction).
- **Its own panel, the 11th (#181).** A trigger isn't a kind of commit — selecting it mutes the granular scan — so it left the commit section for a `device--trigger` panel carrying the on/off switch, a real momentary **record button** (`#trigRecordBtn`, #196), and the live params. `triggers on/off` is the counterpart of **SCAN** — it silences firing without touching a recording, and the gate keeps tracking `_inside` while muted so unmuting doesn't bang whatever the cursor is resting on. Wiring lives in **`js/ui-trigger.js`**. On the sphere, armed strokes draw as decimated outlines (shared 240-projection budget) that brighten on proximity, with a playhead ring on whatever is sounding — no per-trigger halo, since the cursor's own brush ring already shows the reach.
- **LED feedback: a `strike` pattern in electric cyan `#00E5FF` (#197–#198).** One 45 ms full-brightness stab — two messages at the default count, the cheapest thing available, reading percussively rather than as a blink — fired from the gate enter edge, not when the audio graph catches up. A looping trigger stabs on **every wrap** (the wrap index falls straight out of the playhead maths already computed), rate-limited to a 120 ms floor. The row is `priority: true` — the first such row — so a trigger firing cancels an in-flight commit/undo flash instead of being dropped, because a trigger is the performer's primary action feedback.
- **Bindable through the shared `ACTIONS` table:** `trace_trigger` (hold, `/trace/trigger`), `trig_toggle` (`/trigger/mute`) and `trigger_chop` (`/trigger/chop`) — so ⇧space, a pad, a pedal and Max all reach the same code. Session export collapsed a trigger to **a strokeId plus a colour** (audio, particles and params all already ride in the payload once); `p.trig` on the particle carries the type with the material.
- **Verified headless — new `scripts/trigger-audit.js`, 48 assertions** covering both edges, the hysteresis band from both directions, the rearm window, `start`/`dwell`/`retrig`/`chop` behaviour, erase-trim and split, the candidate-pool typing through a real `__testCandidatePool` seam in `grain.js`, and the cost table. **`docs/TRIGGER-TOOL-PLAN.md`** carries the full design record and the scoped-out list. **Not yet played on the rig (#180)** — the gate is verified as geometry; radius 8°, hysteresis 1.15×, rearm 120 ms are starting guesses only a turntable can settle.

#### Grain envelope — proportional vs absolute fade, and a real fadeRatio
- **`fadeRatio` is finally wired through the worklet.** The hann and triangle tables span the whole grain, so before this every grain was permanently the `fr = 0.5` case — full gain only at the exact centre — which is why a transient had to sit mid-grain to be heard at level and why the rect curve sounded so much more present on a hit. Below 0.5 the ramps keep their shape and a unity sustain opens between them. New **`fade %` / `fade ms` unit toggle** (`fadeMode` proportional vs absolute): a percentage ramp scales with grain length, a millisecond ramp stays constant while length varies (better for percussion). A 2 ms minimum ramp (`MIN_FADE_S`) applies in both units unless you set exactly 0.
- **`start ±` — per-grain read-offset jitter (`startJitter`, in seconds).** Markers sit on a fixed clock, so without it a grain can only begin exactly on one; this widens each grain's reach into the audio between markers (guide: half the drop rate).

#### Cursor axis source — sensor | locked | mapped
- **Azimuth and elevation each get a source control** replacing the old lock toggles: `sensor` (the x-imu3 drives it), `locked` (frozen at the held value), or `mapped` (a cursor row in the mapping table drives it). A `cursor` output kind in `sensor-mapping.js` writes azimuth/elevation in degrees; the axis only listens while its `S.<x>Source` is `mapped`, and switching to `mapped` picks up the live value instantly. New **`absInput` conditioning** folds a ± symmetric axis at zero (`|x|`) so a circular euler axis read as −179/+179 at a half-turn stops landing on whichever linear end it feels like. OSC: `/cursor/az_source` `/el_source` (cycle on bang, or set `sensor|locked|mapped`), replacing `/cursor/lock_az` `/lock_el`.

#### Max tooling
- New Max patches and scripts for the multi-station / bearing work: `max/radius.js`, `radius-panel.maxpat`, `radius-probe.maxpat`, `plate-analysis.js`, `max2mubone.maxpat`, `pedals.js` / `pedals.maxpat`, and updated x-imu3 patches.

### Changed
- **Master volume ceiling raised +6 → +18 dB** (both range inputs, `min="-60" max="18" step="0.1"`, default −6). Headroom for quiet material through a PA.
- **Recording seal fade cut 50 ms → 5 ms, and it was destructive (#199).** The 50 ms *squared* fade written into the samples at seal time buried the attack of every take permanently — 25 ms in you were still 12 dB down. This is the one change here not scoped to triggers: every live recording seals through that path. Measured, a transient N ms into a take now reads: 5 ms → 0 dB (was −56.9), 10 ms → 0 dB (was −38.8). One-number revert if a click turns up.
- **Start declick unified to 3 ms (#200).** The 35 ms playback fade-in (inherited from the loop path, where nothing about a loop start is an attack) ate a woodblock on beat one. Triggers and a loop's first pass now get a 3 ms declick (`TRIGGER_DECLICK_S` → `DECLICK_S`, no longer trigger-specific); loops keep `buildLoopPayload`'s 30 ms *tail* crossfade for the wrap, which was always fine.
- **Paint deposit came off the render frame.** The old `PAINT_INTERVAL = 3` every-3rd-frame gate is gone; `paint-ticker.js` now runs a free-running 200 Hz poll depositing at a fixed **20 Hz** (`_DEF_INTERVAL_MS = 50`), adjustable 5–500 ms at runtime and independent of frame rate and input source. Marker `align` (backdate) defaults to **0** — the grain starts at the marker — with the first-after-silence marker owned by the gate lookback instead. Persisted as `paintTickerMs` / `paintAlignMs`.
- **Gate loudness moved to a dedicated 2048-sample analyser** (`GATE_FFT_SIZE`) tapped off the input gain, peak-held at 30 Hz by elapsed time. At the old shared 256-sample window against a 33 ms tick, 84% of the timeline was never observed and a percussive attack had ~1-in-6 odds of being seen — the gate then opened on the decay. The paint-gate meter now also appears in the compact audio-settings modal (`apGateMeter`/`apPaintGateSlider` as a mirror pair), and `/gate/threshold` is rescaled to **0–1** (was 0–0.06), compared against `max(rms, 0.7·peak)` on a curved scale.
- **Renderer hot loop is allocation-free.** Per-particle projection uses a reusable `projectInto()` scratch instead of `project()`, which allocated a result object and recomputed `Math.tan(fov/2)` on every call — 600k allocations/sec of GC pressure at 20k particles × 30 fps. Glow results are cached per particle per frame.
- **Preset and patch-table pass** (`ui-presets.js`, `ui-patch-table.js`) carried in with the Aug 5 work.
- **`EXPORT_VERSION` 5 → 6** — loop and trigger particles now serialise as value tuples, not object-per-particle (see Fixed #178); pre-v6 files keep the old read path.
- **`browser-audit.js` modal-count assertion 12 → 11 (#179)** after #169 deleted `cameraModal`.
- **Dead pre-worklet envelope machinery deleted** — `HANN_LEN`/`buildEnvelopeCurves`/`rebuildGrainCurves` were the main-thread envelope from before the worklet; `rebuildGrainCurves()` still ran on every volume CC and preset change, allocating two `Float32Array(128)` per call, while `S.GRAIN_ATTACK_CURVE`/`RELEASE_CURVE` were written but never read. Also removed the phantom "spatial index rebuild" timing row that described a module that doesn't exist.

### Fixed
- **Every imported loop came back silent (#178).** `ui-export.js` wrote `particleIndices` as `S.particles.indexOf(p)`, but `buildLoopPayload` replaces its particles with **detached copies**, so `indexOf` returned −1 for every one; import filtered them all out and the scheduler skipped the empty slot — buffer intact, no playhead, no marker. Playhead slots were hit identically. Now stored as `[lon, lat, grainStart, grainDuration]` values. Two prior export audits missed it because both were looking for fields that were absent or stale; this field was present, plausibly named and structurally valid, and only its *meaning* was wrong (§ E9 in the export/import audit): an index is a reference into one specific array — only serialise one if the objects are actually in it.
- **OSC latched ON when Max sent `1` then `0` back to back (audit O1/O2).** A two-edge controller's release `0` now drops before anything acts on it; `_VALUED_TRIGGERS` (`/cursor/scan`, `/trigger/mute`, `/trigger/chop`) are exempt because for them a `0` is a command, not a release edge — without that exemption `/trigger/chop 0` (chop off) would be silently swallowed while `1` worked. Full inbound audit — every advertised address reaches a handler and every handler a real function, with the edge/type findings — in **`docs/OSC-AUDIT-2026-08.md`** (harness `scripts/osc-audit.js`).
- **`browser-audit.js` had four checks failing on every run (#179)** — a stale modal count from a deleted modal, in three places. A harness that is always red hides the next real regression, which is most of the reason it exists.
