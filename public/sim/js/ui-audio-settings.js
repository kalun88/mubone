...
// Reusable helper — wire any button to save all defaults with flash feedback
export function wireSaveDefaultBtn(btnId) {
  const btn = document.getElementById(btnId);
  if (!btn) return;
  btn.addEventListener('click', () => {
    const ok = saveAllDefaults();
    btn.textContent = ok ? 'saved ✓' : 'error';
    btn.classList.add('as-btn--flash');
    setTimeout(() => {
      btn.textContent = 'save as default';
      btn.classList.remove('as-btn--flash');
    }, 1200);
  });
}
...
export function initAudioSettings() {
  ...
  // Save as default — persists all settings to localStorage
  wireSaveDefaultBtn('asSaveDefaultsBtn');
}
