...
import { S, PRESETS, MAX_SEEDS, rebuildGrainCurves } from './state.js';
...
// seed(s) along a smooth↔agitated axis. morphT=0.5 = planted snapshot,
// morphT=1.0 = max agitated, morphT=0.0 = max smooth.
//
// The agitation axis applies additive deltas to seed.grainOverrides:
...
export function updateGestureMorph() {
  ...
  // In focus-seed mode with crossfade: distribute morph by distance weights.
  // Otherwise: morph the single nearest seed.
  const morphTargets = []; // [{ seed, weight }]
  ...
  for (const { seed, weight } of morphTargets) {
    ...
    if (S.morphHoldMode === 'elastic' && Math.abs(velocityDelta) < 0.0001) {
      ...
      seed.morphT += (0.5 - seed.morphT) * recoveryRate;
    }

    // Compute agitation axis deltas and write to seed.grainOverrides
    const deviation = (seed.morphT - 0.5) * 2; // -1 to +1
