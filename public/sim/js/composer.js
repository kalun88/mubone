...
import { togglePinMute, pinFadeIn, pinFadeOut } from './pins.js';
...
// The pins' In / Out (Settings › Pins, pins.js pinFadeIn / pinFadeOut) lengthen it:
 ...
 *  (2026-09-05) and the mute node ramps — over the pins' Out on the
 *  way down and In on the way up, never under MUTE_RAMP_S. Waiting for
...
export function setLoopMuted(seq, muted) {
  ...
  const ramp = Math.max(MUTE_RAMP_S, muted ? pinFadeOut(seq) : pinFadeIn());
  ...
  // its share of the In rather than the whole of it from wherever it was.
...
export function setCloudPlaying(seed, playing) {
  ...
  if (playing) {
    ...
    seed._releasingAt    = 0;
    // The unmute rides the pins' In (Settings › Pins, pins.js pinFadeIn),
    // resumed from the level the mute left it at rather than from silence: the
    // attack is t³ (grain.js), so the time already "spent" is fadeIn·∛gain.
    const fi = pinFadeIn();
  ...
  // it when the release lands. With an Out the mute IS that release,
...
export function isCommitOn(c) {
  ...
  // its Out is still running (2026-09-16) — so applyMix() sees an unmute
