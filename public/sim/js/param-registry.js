...
export const PARAM_REGISTRY = [
  ...
  { key: 'filterOn',   label: 'filter',       group: 'grain', type: 'boolean',
    get: () => S.grainOverrides.filterOn ?? S.grainParams.filterOn ?? false,
    set: v  => { S.grainOverrides.filterOn = !!v; },
    fmt: v  => v ? 'on' : 'off',
    parse: s => parseBool(s) },
  { key: 'filterMode', label: 'filter type',  group: 'grain', type: 'enum', options: ['lp', 'bp', 'hp'],
    get: () => S.grainOverrides.filterMode ?? S.grainParams.filterMode ?? 'lp',
    set: v  => { S.grainOverrides.filterMode = v; },
    fmt: v  => v,
    parse: s => ['lp', 'bp', 'hp'].includes(s.trim()) ? s.trim() : null },
  { key: 'cutoff',     label: 'cutoff',       group: 'grain', type: 'number',
    get: () => S.grainOverrides.cutoff ?? S.grainParams.cutoff ?? 1000,
    set: v  => { S.grainOverrides.cutoff = v; },
  ...
  { key: 'res',        label: 'resonance',    group: 'grain', type: 'number',
    get: () => S.grainOverrides.res ?? S.grainParams.res ?? 0,
    set: v  => { S.grainOverrides.res = v; },
    fmt: v  => Math.round(v * 100) + '%',
    parse: s => { const v = parseFloat(s.replace('%', '')) / 100; return isNaN(v) ? null : Math.max(0, Math.min(1, v)); } },
