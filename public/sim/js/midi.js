...
const ACTIONS = [
  ...
  { id: 'grain_startj', label: 'start jitter',              key: '—',  osc: '/grain/startjitter', fmt: 'float 0–2 s',       type: 'cc',
    ccFn: v => { S.grainOverrides.startJitter = (v / 127) * 2; S.syncGrainControlsUI?.(); } },
  { id: 'grain_spray',  label: 'spray count',               key: '—',  osc: '/grain/spray',       fmt: 'int 1–8',           type: 'cc',
    ccFn: v => { S.grainOverrides.sprayCount = Math.max(1, Math.round(1 + (v / 127) * 7)); S.syncGrainControlsUI?.(); } },
  { id: 'grain_sprayw', label: 'spray spread',              key: '—',  osc: '/grain/sprayspread', fmt: 'float 0–1',         type: 'cc',
    ccFn: v => { S.grainOverrides.spraySpread = v / 127; S.syncGrainControlsUI?.(); } },
